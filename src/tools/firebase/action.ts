...
export const onSnap = (name:string, callback:any, id:string|null = null) => {
    if( id ) {
...
export const actionSave = async (
    ...
    try{
        if( id ) updateDoc( doc(db, name, id), data )
        else addDoc( collection(db, name), data )
        return true
    } catch( error ) {
        console.log( error )
        return false
...
export const actionSearch = async (name:string, target:string, condition:any, value:any) => {
